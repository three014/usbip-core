...
where
    ...
    T: DeserializeOwned,
{
    struct BufferVisitor<const N: usize, T>(PhantomData<T>);
    impl<'de, const N: usize, T> Visitor<'de> for BufferVisitor<N, T>
    where
        T: DeserializeOwned,
    {
        type Value = [T; N];
    ...
    deserializer.deserialize_tuple(N, BufferVisitor::<N, T>(PhantomData))
}
